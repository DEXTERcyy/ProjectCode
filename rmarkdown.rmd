...
The raw dataset from **n_starvation.rds** consists of three subsets: **otu_table**, **tax_table** and **sam_data**. Split by nitro condition by **growthCondition**.
...
head(otu_Ab_Nplus,n = c(5,5))
head(otu_Ab_Nminus,n = c(5,5))
```
...
network_Nplus <- network_results$Nplus
network_Nminus <- network_results$Nminus
head(network_Nplus,n = c(5,5))
head(network_Nminus,n = c(5,5))
```
...
### Plot network
Visualize the edge network seperately and colorized by Phylum.
```{r Network_Plot 1}
...
Now, synthesize 50 dataset based on the origin/true aboundance data.
...
for (metric_name in unique(results_df_long$metric)) {
    ...
    p1 <- ggplot(plot_data, aes(x = matrix_id, y = value, fill = group)) +
        ...
        theme_bw()
    print(p1)
    p2 <- ggplot(plot_data, aes(x = group, y = value, fill = group)) +
        geom_boxplot() +
