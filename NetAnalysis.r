...
# %%
myNetConstruct <- function(assoMat1, assoMat2, sparsMethod = "none")
{
  set.seed(10010)
  assoMat1 <- assoMat1
  assoMat2 <- assoMat2
  ...
  # Create edge list
  g1 <- igraph::graph_from_adjacency_matrix(adjaMat1, weighted = weighted,
  ...
  if (is.null(igraph::E(g1)$weight)) {
    ...
    edgelist1 <- data.frame(igraph::get.edgelist(g1))
  ...
  g2 <- igraph::graph_from_adjacency_matrix(adjaMat2, weighted = weighted, 
        mode = "undirected", diag = FALSE)

  if (is.null(igraph::E(g2)$weight)) {
    isempty2 <- TRUE
    edgelist2 <- NULL
  } else {
    isempty2 <- FALSE

    edgelist2 <- data.frame(igraph::get.edgelist(g2))
...
plot(NetRes, 
  ...
  nodeSizeSpread = 3, # Adjust nodeSizeSpread for compactness
  cexNodes = 4, # Increase node size
